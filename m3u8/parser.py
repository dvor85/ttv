...
def parse(content, strict=False, custom_tags_parser=None):
    ...
    for line in string_to_lines(content):
        ...
        if line.startswith(protocol.extgrp):
            _parse_grp(line, state)
...
def _parse_grp(line, state):
    if 'segment' not in state:
        state['segment'] = {}
    state['segment']['group'] = line.split(':')[1].strip()
